...
if (isset($_POST['login'], $_POST['password']) && !empty($_POST['login'])) {
    if (strlen($_POST['login']) && $controller->change_login($login, $_POST['login'], $_POST['password']))
...
if (isset($_POST['old_pw'], $_POST['new_pw'], $_POST['confirm_new_pw'])) {
    if (!preg_match("/[0-9]*/", $_POST['new_pw']))
        $controller->redirect_get("/view/account.php", ["edit_password" => "invalid_pw"]);
    else if ($controller->change_password($login, $_POST['old_pw'], $_POST['new_pw'], $_POST['confirm_new_pw']))
...
if (isset($_POST['notif'])) {
    ...
    else if ($controller->switch_notif($_SESSION['user_id'], $_POST['notif']))
        $controller->redirect_get("/view/account.php", ['edit_notif' => 'success']);
    else
        $controller->redirect_get("/view/account.php", ['edit_notif' => 'fail']);
}
